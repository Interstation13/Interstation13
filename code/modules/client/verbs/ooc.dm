...
/client/verb/looc(msg as text)
	...
	if(!holder)
		if(!GLOB.looc_allowed)
			...
			return
		if(mob.stat == DEAD)
			to_chat(usr, "<span class='danger'>You are not alive enough for LOOC.</span>")
			return
		if(prefs.muted & MUTE_LOOC)
			to_chat(src, "<span class='danger'>You cannot use LOOC (muted).</span>")
			return
		if(is_banned_from(ckey, "OOC"))
			to_chat(src, "<span class='danger'>You have been banned from all OOC.</span>")
			return
	...
	var/message_regular = "<span class='looc'>LOOC: [mob.name]: [msg]</span>"
