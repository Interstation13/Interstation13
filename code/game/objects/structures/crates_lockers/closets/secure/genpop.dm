...
/obj/structure/closet/secure_closet/genpop/attackby(obj/item/W, mob/user, params)
	if(!broken && locked && registered_id != null && W == registered_id && registered_id.released == TRUE) //Prisoner opening
...
/obj/structure/closet/secure_closet/genpop/attack_hand(mob/user)
	if(isliving(user))
		...
		if(!(L.mobility_flags & MOBILITY_STAND) && get_dist(src, user) > 0)
			return
